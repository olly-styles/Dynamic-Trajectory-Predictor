# File taken from - https://github.com/bryanyzhu/two-stream-pytorch
...
class Scale(object):
    ...
    def __call__(self, clips):
        ...
        if is_color:
            num_imgs = int(c / 3)
            scaled_clips = np.zeros((new_h,new_w),dtype='uint8')
            ...
            num_imgs = int(c / 1)
            scaled_clips = np.zeros((new_h,new_w,c),dtype='uint8')
            for frame_id in range(num_imgs):
                cur_img = clips[:,:,frame_id:frame_id+1]
                #cur_img = np.squeeze(cur_img)
                scaled_clips[:,:,frame_id] = cv2.resize(cur_img, (new_w, new_h), self.interpolation)
    ...
    def __call__(self, clips):
        ...
        for attempt in range(10):
            ...
            if new_w <= w and new_h <= h:
                ...
                scaled_clips = np.zeros((self.size,self.size,c),dtype='uint8')
                if is_color:
                    num_imgs = int(c / 3)
                    for frame_id in range(num_imgs):
                        cur_img = clips[:,:,frame_id*3:frame_id*3+3]
...
class MultiScaleCrop(object):
    ...
    def __call__(self, clips):
        ...
        scaled_clips = np.zeros((self.height,self.width,c),dtype='uint8')
        if is_color:
            num_imgs = int(c / 3)
            for frame_id in range(num_imgs):
                cur_img = clips[:,:,frame_id*3:frame_id*3+3]
